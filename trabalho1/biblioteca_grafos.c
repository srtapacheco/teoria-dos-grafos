...
int compara(const void *a, const void *b) // Função auxiliar para comparar dois inteiros, usada na ordenação dos graus
{
...
void processa_arquivo_grafo(const char *nome_arquivo, const char *representacao, int vertice_inicial)
{
...
int main(int argc, char *argv[])
{
    if (argc < 4)
    {
        printf("Uso: %s <arquivo_grafo> <representacao (matriz|lista)> <vertice_inicial>\n", argv[0]);
        return 1;
    }
    ...
    processa_arquivo_grafo(nome_arquivo, representacao, vertice_inicial);
    return 0;
}
